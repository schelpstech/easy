...
<div class="row g-4"><div class="col-xl-7"><section class="staff-card">
    ...
    <?php if ($settings['transport'] === 'mail' && !function_exists('mail')): ?><div class="alert alert-warning">PHP <code>mail()</code> is unavailable in this website's PHP runtime. Select <strong>Authenticated SMTP</strong> and save your SMTP details before testing. Cron may use a separate PHP configuration.</div><?php endif; ?>
    <div class="mb-3"><label for="transport">Transport</label><select class="form-select" id="transport" name="transport"><option value="smtp" <?= $settings['transport'] === 'smtp' ? 'selected' : '' ?>>Authenticated SMTP</option><option value="mail" <?= $settings['transport'] === 'mail' ? 'selected' : '' ?>>Server mail — PHP mail() (no SMTP login)</option></select><small class="text-muted">Select Authenticated SMTP to use the host, username and password below. Server mail ignores these SMTP fields and requires your hosting mail system.</small></div>
